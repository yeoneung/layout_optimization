"""
Classical side of the comparison, on the same held-out instances v1 used.

First it reproduces the v1 SA number as a regression check, then it runs the
...
def heldout(spec, n_inst, seed):
    """Exactly the draw v1's evaluate.py uses, so numbers stay comparable."""
...
def main():
    ...
    # --- regression check against v1's headline SA number -------------------
    ...
    row = report("SA 45k + polish (v1 setup)", obj, r["x"], r["y"], r["evals"],
                 r["wall_time"], rows, N)
    if a.scenario in V1_REPORTED and a.flip_adj:
        j, f = V1_REPORTED[a.scenario]
        print(f"    [regression] v1 reported J={j:.1f} at {f*100:.0f}% feasible for "
              f"this configuration; v2 gives J={row['mean']:.1f} at "
