"""
The improvement MDP, with the three things v1's formulation was missing.

1. Reward.  v1 used r_t = J(s_{t+1}) - J(s_t).  That telescopes to J(s_T) - J(s_0),
   ...
   v1's own discount sweep shows exactly that: gamma=0 scores 2409, gamma=0.995
...
2. Move set.  v1's agent could only translate one room by a fixed step, while the
...
3. Accept/reject.  In v1 every sampled action was applied unconditionally, so an
...
class ImproveEnv:
    ...
    def step(self, a):
        ...
        else:                                    # v1's delta-of-objective reward
