"""
Tracks A and C plus the legacy control, run back to back through one trainer.
...
of separately tuned systems.
  ...
  A0g delta reward, translate only, gamma=0.0     <- legacy's own best discount
...
RUNS = [
    dict(tag="A0_legacy_delta_g995", reward="delta", ops="translate", gamma=0.995, lam=0.96),
    dict(tag="A0_legacy_delta_g0", reward="delta", ops="translate", gamma=0.0, lam=0.0),
