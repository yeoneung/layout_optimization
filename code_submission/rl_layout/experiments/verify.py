"""
Chain-of-trust check for the batched objective.

    comb_high.objective()  ==  layout_env.LayoutObjective  ==  core.BatchObjective

This script compares the batched and legacy objectives on both reference
scenarios, on random layouts, on grid-aligned layouts, and on layouts with heavy
overlap. It also checks the batched objective directly against the original
...
def check_against_legacy(scenario, flip, rng, B=64):
...
def check_against_original(scenario, rng, B=8):
    """Compare directly with the original pure-Python objective()."""
...
def main():
    ...
    print("batched BatchObjective  vs  legacy LayoutObjective")
    worst = 0.0
    for scenario in ("comb_high", "hospital"):
        for flip in (False, True):
            worst = max(worst, check_against_legacy(scenario, flip, rng))
    print(f"\n  worst disagreement over all cases: {worst:.3e}")
    assert worst < 1e-9, "batched objective does not match legacy"

    print("\nbatched BatchObjective  vs  original pure-Python objective()")
    worst2 = 0.0
    for scenario in ("comb_high", "hospital"):
        worst2 = max(worst2, check_against_original(scenario, rng))
    assert worst2 < 1e-6, "batched objective does not match the original scripts"

    print("\nprocedural instance generator")
    check_generator(rng)
    print("\nOK - batched objective is the same function the SA baseline optimizes.")
